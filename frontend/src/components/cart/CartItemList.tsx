  ...
  if (cartItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-6 text-center bg-white border border-slate-200 rounded-2xl">
        ...
        />
        <h2 className="text-lg font-semibold text-slate-700 mt-5">
          Your cart is empty
        </h2>
      ...
      {cartItems.map((item) => (
        <div key={item.id} className="flex flex-col md:flex-row gap-4 p-5">
          <Link
            ...
            className="w-[110px] h-[110px] shrink-0 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-center"
          >
            <img
              src={item.image}
              alt={item.title}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
              <div className="min-w-0">
                <Link
                  href={`/products/${item.id}`}
                  className="text-sm md:text-base font-semibold text-slate-800 hover:text-blue-600 transition-colors line-clamp-2"
                >
                  {item.title}
                </Link>
                <div className="mt-2 flex flex-col gap-1 text-xs text-slate-500">
                  ...
                  <p>
                    Seller:{" "}
                    <span className="text-slate-700">Best Factory LLC</span>
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3 mt-4">
                  <button
                    ...
                    className="cursor-pointer inline-flex items-center gap-1.5 text-xs text-red-500 hover:text-red-600 font-medium"
                  >
                    <FaTrash size={11} /> Remove
                  </button>
                  <button
                    onClick={() => saveForLater(item.id)}
                    className="cursor-pointer inline-flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-700 font-medium"
                  >
