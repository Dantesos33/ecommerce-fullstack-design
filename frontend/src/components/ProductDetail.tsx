      ...
      {products.slice(0, 5).map((p) => (
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-xs text-slate-700 group-hover:text-blue-600 transition-colors line-clamp-2 leading-snug">
              {p.title}
            </p>
...
const ProductDetail = ({ product, relatedProducts }: ProductDetailProps) => {
  const thumbnails = [
    product.image,
    ...relatedProducts.slice(0, 5).map((p) => p.image),
  ];
  ...
  const prevImg = () =>
    setSelectedImg((i) => (i - 1 + thumbnails.length) % thumbnails.length);
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex flex-col gap-3 lg:w-[280px] shrink-0">
              ...
              <div className="relative flex items-center justify-center bg-slate-50 border border-slate-100 rounded-xl h-[260px] md:h-[300px]">
                ...
                <button
                  onClick={prevImg}
                  className="cursor-pointer absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 bg-white border border-slate-200 rounded-full flex items-center justify-center shadow-sm hover:bg-slate-50"
                >
                  <FaChevronLeft size={10} className="text-slate-500" />
                </button>
                <button
                  onClick={nextImg}
                  className="cursor-pointer absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 bg-white border border-slate-200 rounded-full flex items-center justify-center shadow-sm hover:bg-slate-50"
                >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 flex-wrap mb-4 text-xs text-slate-500">
                ...
                <span className="flex items-center gap-1">
                  <FaRegCommentDots size={11} /> {product.reviews} reviews
                </span>
                <span>·</span>
                <span>{154} sold</span>
              </div>
              ...
              <div className="flex rounded-xl overflow-hidden border border-slate-200 mb-4 w-full max-w-sm">
                {priceTiers.map((tier, i) => (
                  <div
                    ...
                    style={{
                      backgroundColor: tier.highlight ? "#fff7ed" : "#ffffff",
                    }}
                  >
              ...
              <div className="flex items-center gap-3 mt-5">
                <button
                  ...
                  className={`cursor-pointer flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
                    inCart
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden mb-5">
          <div className="flex border-b border-slate-100 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            {(["description", "reviews", "shipping", "seller"] as const).map(
              (tab) => (
                <button
                  key={tab}
            ...
            <div className="flex-1 min-w-0">
                  ...
                  <table className="w-full text-sm border-collapse">
                    <tbody>
                      {tableSpecs.map(({ label, value }, i) => (
                        ...
                        >
                          <td className="px-4 py-2.5 text-slate-500 w-[160px] border border-slate-100">
                            {label}
                          </td>
                          <td className="px-4 py-2.5 text-slate-700 border border-slate-100">
                            {value}
                          </td>
                        </tr>
                      ))}
              ...
              {activeTab === "reviews" && (
                <p className="text-sm text-slate-600">
                  {product.reviews} customer reviews · Average rating{" "}
                  {product.rating}/10
                </p>
              )}
              {activeTab === "shipping" && (
              ...
              {activeTab === "seller" && (
                <p className="text-sm text-slate-600">
                  Guanjoi Trading LLC · Germany, Berlin · Verified Seller
                </p>
              )}
            </div>
              ...
              {relatedProducts.map((p) => (
                ...
                >
                  ...
                  <div className="px-2.5 pb-2.5 border-t border-slate-100">
                    <p className="text-xs text-slate-500 line-clamp-2 leading-snug mt-1.5">
                      {p.title}
                    </p>
